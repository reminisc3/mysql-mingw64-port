...
/*
  Helper function for mysql_session_track_get_*() functions.
*/

static int store_data_and_length(LIST *node, const char **data, size_t *length)
{
  if (data)
    *data= node && node->data ? ((LEX_STRING*)(node->data))->str : NULL;
  if (length)
    *length= node && node->data ? ((LEX_STRING*)(node->data))->length : 0;

  return node && node->data ? 0 : 1;
}
                                          ...
                                          size_t *length)
{
  ...
  if (!info || type > SESSION_TRACK_END || !(info->info_list[type].head_node))
    return store_data_and_length(NULL, data, length);
                                         ...
                                         size_t *length)
{
  ...
  if (!info || type > SESSION_TRACK_END
            || !(info->info_list[type].current_node))
    return store_data_and_length(NULL, data, length);

  ret= store_data_and_length(info->info_list[type].current_node,
                             data, length);
